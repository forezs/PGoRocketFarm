...
url = 'https://sgpokemap.com/pokestop.php?time='
needed, already = [24, 25, 12, 13, 16, 17], []
end = False
current_time, reopen_time, next_time, back_time = time(), time(), time(), time()
...
class MainAction():
    def __init__(self):
        ...
        self.open_pokestop_loc = (x // 2, y//2)
    ...
    def gotcha(self):
        subprocess.run(f'adb -s {device_id} shell am start -a android.intent.action.VIEW -d "https://pk.md/55.583447,37.681365"', stdout=subprocess.PIPE)
        sleep(5)
        self.swipe(self.throw[0], self.throw[1], 300)
...
class MainDetector:
    ...
    def timers_check(self):
        global current_time, reopen_time, next_time, back_time
        current_time = time()
        ...
        elif current_time - back_time > 15 and not end:
            print('[INFO] Clicked back..')
            action.click_back()
            action.click_back()
            back_time = time()
    ...
    def open_pokestop(self):
        if not end:
            ...
            stop_img = stop_img[y//3:int(y/1.5),x//3:int(x/1.5)]
    ...
    def close_pokestop(self):
        if not end:
            ...
            close_img = close_img[y//5:y//3,x//3:int(x/1.5)]
    ...
    def battle(self):
        global next_time, reopen_time, count, end, back_time
        ...
        for i in words:
            if fuzz.WRatio(i, 'choose your party') > 90:
                ...
                next_time, reopen_time, back_time = time(), time(), time()
                end = True

    def detect_poke(self):
        global next_time, reopen_time, count, end, back_time, iv_counter
        img = action.make_screencap()
        x, y = img.shape[:2][::-1]
        img = img[y//4:y//2, x//4:int(x/1.2)]

        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        _, img_binary = cv2.threshold(img, 130, 255, cv2.THRESH_TOZERO)
        img_binary = abs(255-img_binary)

        words = pytesseract.image_to_string(img_binary).split('\n')
        for i in words:
            iv = re.search(r'\d{,2}/\d{,2}/\d{,2}', i)
            if iv:
                while True:
